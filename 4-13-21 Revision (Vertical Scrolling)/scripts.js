...
var slide_parent = document.getElementById("slide_container");
      ...
      function explode(){
		...
		if( !($('#side_menu_wrapper').hasClass('active')) ) {
			...
			// $('.fill_change').addClass('blackFill');
			  ...
			  TweenLite.to("#preview_slide_wrapper", 1.5, {marginTop: "-100%"});

			 TweenLite.to("#preview_slide_wrapper_phantom", .1, {autoAlpha: "1"});

			 TweenLite.to("#content", .75, {autoAlpha: '1', marginTop: "0"});
	...
	if ( $('#content').hasClass('revealed') ) {
		...
		TweenLite.to("#preview_slide_wrapper", 1, {marginTop: "0"});

		TweenLite.to("#preview_slide_wrapper_phantom", .1, {autoAlpha: "0"});

		TweenLite.to("#content", .75, {autoAlpha: '0', marginTop: "100%"});
...
window.addEventListener("wheel", function(w){ // or window.addEventListener("scroll"....

   var st = w.wheelDelta;
   if (st > 0){

	   	if( $('.slide').eq(0).hasClass('current') && !( $('.slide').eq(0).hasClass('animating') ) ){

	   		exitExplode();

	   	}

    	if( !( $(".slide").hasClass('animating') ) ) {
...
function move(e){
	...
	slide.not( current ).removeClass('animating');
